...
- [Development guidelines](#development-guidelines)
    - [General Constella Core](#general-constella-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Constella Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play constellas" on the test network, if you
...
Constella Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/constella-project/constella/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_constella
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_constella --log_level=test_suite
...
# A coverage report will now be accessible at `./test_constella.coverage/index.html`.
```
...
To profile a running constellad process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep constellad` -- sleep 60
```
...
Constella Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Constella Core, where everyone uses
...
pay attention to for reviewers of Constella Core code.

General Constella Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Constella Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Constella Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Constella Core so that they can be integrated
...
file descriptors it will cause problems with Constella's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Constella's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Constella Core, the wallet was always
