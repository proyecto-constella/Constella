Constella Core version 0.13.2 is now available from:

  <https://download.constella.org/constella-0.13.2.1/>
...
Previous versions of Constella Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Constella Core was to raise the default minimum
relay fee.

Constella Core 0.13.2 will have a strict maximum size on the mempool. The
...
Constella Core 0.13.2 also introduces new default policy limits on the length and
...
Constella Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Constella Core will relay transactions with
...
entirely in Constella Core 0.13, and it is at this time undecided whether the
...
Constella Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Constella Core automatically creates a hidden service to listen on, without
manual configuration. Constella Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Constella Core is listening, and
...
Core to use floating fees. By default, Constella Core will use floating
fees.
...
At all times, Constella Core will cap fees at `-maxtxfee=<x>` (default:
0.10) XCL.
Furthermore, Constella Core will never create transactions paying less than
...
Various code modernizations have been done. The Constella Core code base has
...
HD wallets are incompatible with older versions of Constella Core.
...
The two phases are distinct in the Constella-Qt GUI. During the first one,
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  note that Constella Core does not provide an option to use this capability as
...
- **Script versioning:** Segwit makes it easy for future soft forks to allow
  ...
  currently being researched by Bitcoin and Constella Core contributors that may
...
client to Constella Core 0.13.2 to signal for segwit support. When a
...
Since Constella Core 0.10.0, nodes have defaulted to only relaying and
